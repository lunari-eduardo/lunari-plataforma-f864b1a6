...
AS $$
  WITH sess AS (
    SELECT id, COALESCE(valor_total,0) AS valor_total, COALESCE(valor_pago,0) AS valor_pago
  ...
  cred_ger AS (
    SELECT COALESCE(SUM(l.valor), 0) AS v
      ...
      JOIN public.clientes_sessoes s ON s.id::text = l.session_id_origem
  ...
  cred_uso AS (
    SELECT COALESCE(SUM(ABS(l.valor)), 0) AS v
      FROM public.cliente_creditos_ledger l
      JOIN public.clientes_sessoes s ON s.id::text = l.session_id_consumo
     WHERE l.user_id = p_user_id
       AND s.data_sessao BETWEEN p_start AND p_end
  ),
  caixa AS (
...
GRANT EXECUTE ON FUNCTION public.workflow_month_metrics(uuid, date, date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.workflow_month_metrics(uuid, date, date) TO service_role;
...
AS $$
  SELECT COALESCE(SUM(saldo), 0)
    FROM (
      SELECT cliente_id, GREATEST(SUM(valor), 0) AS saldo
