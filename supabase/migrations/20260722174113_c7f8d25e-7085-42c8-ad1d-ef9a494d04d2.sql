...
BEGIN
  ...
  IF v_pre_selecao THEN
    v_extras_pago := 0;
  ELSE
    -- (a) Componentes explícitos (cobrança com finalidade + componentes)
    SELECT COALESCE(SUM(
      CASE
    ...
    -- (b1) Pagamentos manuais rotulados como :fotos_extras → 100% para extras
    ...
    -- (b2) Pagamentos manuais rotulados como :sessao_e_extras → RATEIO proporcional
    -- (bug antigo: creditava 100% para extras, subestimando extras_pendente
    -- e distorcendo sessao_pago).
    SELECT COALESCE(SUM(t.valor), 0)
    ...
    IF v_total > 0 AND v_extras_liq > 0 THEN
      v_extras_pago_manual := v_extras_pago_manual_fotos
                            + (v_manual_combinado * (v_extras_liq / v_total));
    ELSE
      v_extras_pago_manual := v_extras_pago_manual_fotos;
    END IF;

    -- (c) Rateio proporcional para pagamentos genéricos (sem tag INTENT)
    --     quando o modo é manual/override.
