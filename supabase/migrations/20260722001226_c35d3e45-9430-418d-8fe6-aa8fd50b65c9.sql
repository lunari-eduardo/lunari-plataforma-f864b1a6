-- Ajusta workflow_session_financials para reconhecer pagamentos manuais
-- de fotos extras registrados via capability billing.registerManualPayment
-- (identificados pelo marcador `[INTENT:billing.manual:...:fotos_extras]`
-- ou `[INTENT:...:sessao_e_extras]` na descrição da transação).
--
-- Sem esse ajuste, a baixa manual de extras não abatia extras_pendente,
-- porque `clientes_transacoes` fica sem `cobranca_id` estrutural.
...
BEGIN
  ...
  ELSE
    ...
    IF v_s.galeria_id IS NOT NULL AND v_override THEN
  ...
  IF v_pre_selecao THEN
    v_extras_pago := 0;
  ELSE
    -- (a) Componentes explícitos: cobranças com finalidade extras/combinada
    SELECT COALESCE(SUM(
      CASE
    ...
    -- (b) Pagamentos manuais rotulados como extras via INTENT
    -- (registro sem cobranca_id, criado pelo billing.registerManualPayment).
    ...
    -- (c) Pagamentos genéricos — rateio proporcional (sem galeria OU override)
    IF (v_s.galeria_id IS NULL OR v_override) AND v_total > 0 AND v_extras_liq > 0 THEN
      SELECT COALESCE(SUM(t.valor), 0)
