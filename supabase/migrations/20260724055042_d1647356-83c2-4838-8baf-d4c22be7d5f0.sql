...
BEGIN
  ...
  ELSE
    ...
    -- Bucket 4a: manual tagueado ':sessao]' — prioriza sessão, sangra p/ extras se excedente
    SELECT COALESCE(SUM(t.valor), 0)
      INTO v_sessao_tag
      FROM public.clientes_transacoes t
     WHERE t.user_id = v_s.user_id
    ...
    -- Bucket 4b: genéricos = sem cobrança, sem qualquer tag de escopo
    SELECT COALESCE(SUM(t.valor), 0)
    ...
    -- Quanto da sessão sobra depois de contabilizar pagamentos que não são
    -- waterfall-elegíveis (tudo exceto :sessao] e genéricos).
    DECLARE
    ...
    BEGIN
      ...
      -- :sessao] cobre sessão primeiro, spilla o excedente
      ...
      -- Genéricos cobrem o que sobrar da sessão, depois spillam
