...
DECLARE
  ...
  v_saldo_extras_apos_diretos numeric := 0;
...
BEGIN
  ...
  IF v_pre_selecao THEN
    v_extras_pago := 0;
  ELSE
    -- Bucket 1: cobranças explicitamente marcadas como extras / combinado (componente extras)
    SELECT COALESCE(SUM(
      CASE
    ...
    -- Bucket 2: pagamento manual tagueado ':fotos_extras]' (100% para extras)
    ...
    -- Bucket 3: manual combinado tagueado ':sessao_e_extras]' (rateio proporcional legado)
    SELECT COALESCE(SUM(t.valor), 0)
    ...
    -- Bucket 4: genéricos = sem cobrança, sem qualquer tag de escopo
    -- (inclui pagamentos antigos sem INTENT).
    -- ':sessao]' NUNCA entra aqui: por definição vai 100% para sessão.
    SELECT COALESCE(SUM(t.valor), 0)
    ...
    -- Waterfall: quanto de extras já está coberto por buckets diretos (1 e 2)?
    v_pago_direto_extras := v_extras_pago_componentes + v_extras_pago_manual_fotos;
    v_saldo_extras_apos_diretos := GREATEST(0, v_extras_liq - v_pago_direto_extras);

    -- Combinado tagueado: mantém rateio proporcional (contrato legado explícito)
    ...
    -- GENÉRICOS: SESSÃO PRIMEIRO. Só sangra para extras depois de quitar a sessão.
    -- Sessão pendente considerando o que já foi pago fora dos genéricos:
    DECLARE
      v_pago_nao_generico numeric := v_pago - v_pagamentos_genericos;
      v_sessao_pendente_pre_generico numeric;
    BEGIN
      IF v_pago_nao_generico < 0 THEN v_pago_nao_generico := 0; END IF;
      -- pago não-genérico que sobra depois de cobrir extras diretos:
      v_sessao_pendente_pre_generico := GREATEST(
        0,
        v_sessao_liq - GREATEST(0, v_pago_nao_generico
                                    - v_pago_direto_extras
                                    - v_combinado_para_extras)
      );
      v_generico_para_extras := GREATEST(
        0,
        v_pagamentos_genericos - v_sessao_pendente_pre_generico
      );
    END;
