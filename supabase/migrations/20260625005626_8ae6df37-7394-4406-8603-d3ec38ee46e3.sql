-- Garante que cobranças com finalidade='fotos_extras' não inflem o valor_pago da sessão.
-- Substitui ensure_transaction_on_cobranca_paid para, quando a cobrança for de fotos
-- extras (vinculada a uma galeria), gerar a transação SEM session_id. A receita
-- continua registrada no extrato (clientes_transacoes) e na galeria
-- (galerias.valor_total_vendido / receita_fotos_extras via trigger do Gallery),
-- mas o trigger recompute_session_paid -- que soma por session_id -- a ignora.
--
-- Compat: cobranças sem finalidade (legado) ou com finalidade='sessao' mantêm o
-- comportamento atual (session_id preservado).
...
BEGIN
  IF NEW.status IN ('pago', 'pago_manual') AND (OLD.status IS NULL OR OLD.status NOT IN ('pago', 'pago_manual')) THEN
    ...
    -- Para cobranças de fotos extras NÃO resolvemos session_id: a receita pertence
    -- à galeria, não à sessão. Mantemos cliente_id para o extrato continuar mostrando.
    IF NOT v_is_extras AND NEW.session_id IS NOT NULL THEN
    ...
    -- Só preserva session_id texto para cobranças de sessão. Extras ficam com NULL.
    IF NOT v_is_extras AND NOT v_session_exists THEN
      v_session_text := NEW.session_id;
    END IF;

    -- Primary check: by cobranca_id (idempotência)
    ...
    -- Secondary check (só para sessao): mesma sessão + mesmo valor + mesmo provedor
    IF v_existing_tx IS NULL AND NOT v_is_extras AND v_session_text IS NOT NULL THEN
      SELECT id INTO v_existing_tx
      FROM public.clientes_transacoes
    ...
    IF v_existing_tx IS NULL THEN
      INSERT INTO public.clientes_transacoes (
        ...
        NEW.user_id,
        v_cliente_id,
        -- Extras nunca vinculam à sessão (evita inflar valor_pago)
        CASE WHEN v_is_extras THEN NULL ELSE v_session_text END,
        v_valor_transacao,
        v_valor_liquido,
        v_taxa_gateway,
        v_taxa_antecipacao,
