...
-- 1.a Backfill do FK clientes_sessoes.galeria_id a partir do slug
...
-- 1.b Backfill dos números derivados nas sessões pós-finalização,
-- respeitando gate pré-seleção e overrides manuais.
UPDATE public.clientes_sessoes s
   SET qtd_fotos_extra        = COALESCE(g.total_fotos_extras_vendidas, 0),
       valor_total_foto_extra = COALESCE(g.valor_total_vendido, 0),
       valor_foto_extra       = COALESCE(NULLIF(g.valor_foto_extra,0), s.valor_foto_extra),
       updated_at             = now()
  FROM public.galerias g
 WHERE s.galeria_id = g.id
   AND COALESCE(s.extras_overridden, false) = false
   AND g.status NOT IN ('rascunho','enviado','selecao_iniciada');

-- 1.c Trigger para manter o FK amarrado daqui pra frente
...
DROP TRIGGER IF EXISTS trg_tie_session_galeria_fk ON public.galerias;
CREATE TRIGGER trg_tie_session_galeria_fk
  AFTER INSERT OR UPDATE OF session_id ON public.galerias
  FOR EACH ROW EXECUTE FUNCTION public.tie_session_galeria_fk();

-- 1.d Defesa em profundidade em workflow_session_financials:
-- se o FK estiver NULL, resolver por slug antes de continuar.
...
BEGIN
  ...
  -- Defesa em profundidade: se FK NULL, resolver galeria por slug.
