-- ==============================================================================
-- Migration: 20260827180000_fix_ensure_transaction_prevent_charge_collision.sql
-- Description: Remove a checagem ambigua por (session_id + valor + provedor)
-- em ensure_transaction_on_cobranca_paid. Garante que cobrancas com mesmo valor
-- (ex: sinal R$ 1,00 e fotos extras R$ 1,00, ou parcelas iguais) gerem suas
-- respectivas transacoes em clientes_transacoes sem sofrer colisao/sobrescrita.
-- ==============================================================================
...
BEGIN
  IF NEW.status IN ('pago', 'pago_manual') AND (OLD.status IS NULL OR OLD.status NOT IN ('pago', 'pago_manual')) THEN
    ...
    IF v_repassar_processamento AND v_repassar_antecipacao THEN
      -- Cliente pagou todas as taxas -> Fotografo recebe valor nominal integral, taxa_gateway = 0
      v_taxa_gateway := 0;
      v_taxa_antecipacao := 0;
    ...
    -- Idempotencia estrita 1: por cobranca_id FK
    SELECT id INTO v_existing_tx
    FROM public.clientes_transacoes
    WHERE cobranca_id = NEW.id
    LIMIT 1;

    -- Idempotencia estrita 2: por mencao explicita ao UUID desta cobranca na descricao
