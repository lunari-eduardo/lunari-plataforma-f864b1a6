...
-- Trigger alinhado à mesma fórmula (Opção A + progressivo)
...
BEGIN
  v_produtos := public.calculate_manual_products_total(NEW.produtos_incluidos);
  ...
  IF NEW.galeria_id IS NOT NULL THEN
    ...
    v_qtd := COALESCE(
      NULLIF(COALESCE(v_gal.total_fotos_extras_vendidas, 0), 0),
      COALESCE(NEW.qtd_fotos_extra, 0)
    );
    v_unit_bruto := COALESCE(
    ...
    v_regras := COALESCE(
      CASE WHEN jsonb_typeof(NEW.regras_congeladas) = 'object' THEN NEW.regras_congeladas END,
      v_gal.regras_congeladas
    );
  ELSE
    v_qtd        := COALESCE(NEW.qtd_fotos_extra, 0);
    v_unit_bruto := COALESCE(NEW.valor_foto_extra, 0);
...
CREATE TRIGGER trigger_recalculate_valor_total
  BEFORE INSERT OR UPDATE OF
    valor_base_pacote, valor_total_foto_extra, valor_foto_extra, qtd_fotos_extra,
    valor_adicional, produtos_incluidos, desconto, galeria_id, regras_congeladas
  ON public.clientes_sessoes
  FOR EACH ROW
  EXECUTE FUNCTION public.recalculate_session_valor_total();

-- Sync sessão → galeria também depois de vendas
...
DROP TRIGGER IF EXISTS trg_sync_gallery_extra_price ON public.clientes_sessoes;
CREATE TRIGGER trg_sync_gallery_extra_price
  AFTER UPDATE OF valor_foto_extra ON public.clientes_sessoes
  FOR EACH ROW EXECUTE FUNCTION public.sync_gallery_extra_price_from_session();

-- Backfill: força trigger a rodar em todas as sessões
