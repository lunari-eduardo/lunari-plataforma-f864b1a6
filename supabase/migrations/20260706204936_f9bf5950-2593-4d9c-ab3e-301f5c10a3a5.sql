...
DECLARE
  ...
  v_sess_text      text;
BEGIN
  SELECT s.id, s.user_id, s.session_id AS session_slug, s.valor_base_pacote, s.valor_foto_extra,
  ...
  -- Ledger: session_id_origem/consumo são TEXT (aceitam UUID em string e slugs
  -- legados "workflow-*"). Comparamos com ambos: uuid stringificado e slug.
