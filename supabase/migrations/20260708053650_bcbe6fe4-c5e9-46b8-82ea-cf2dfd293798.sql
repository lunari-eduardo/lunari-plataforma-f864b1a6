...
-- =========================================================================
-- 1) Reescreve workflow_session_financials como FONTE ÚNICA de verdade.
--    Opção A: `desconto` é global (abate base + extras + produtos + adicional).
--    Quando há galeria, extras vêm dela (qtd vendida + preço unitário).
-- =========================================================================
...
BEGIN
  ...
  -- Fonte dos extras: galeria (quando existir) tem precedência.
...
-- =========================================================================
-- 2) Sincronização automática de preço da foto extra: sessão → galeria,
--    apenas enquanto NÃO houver extras vendidas (para não alterar histórico
--    financeiro consumado).
-- =========================================================================
  ...
  IF NEW.galeria_id IS NOT NULL
    ...
    UPDATE public.galerias
     ...
     WHERE id = NEW.galeria_id
       AND COALESCE(total_fotos_extras_vendidas, 0) = 0
