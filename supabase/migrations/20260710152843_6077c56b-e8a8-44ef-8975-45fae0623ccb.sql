...
BEGIN
  ...
  IF v_s.galeria_id IS NOT NULL THEN
    ...
    -- CORREÇÃO: quando a galeria está com seleção finalizada, considerar SEMPRE
    -- MAX(vendidas, selecionadas−incluídas). Isso cobre reabertura da galeria
    -- e nova seleção após pagamento parcial (v_gal.total_fotos_extras_vendidas
    -- fica estável no que foi vendido, mas fotos_selecionadas cresce).
