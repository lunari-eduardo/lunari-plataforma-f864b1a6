-- 1) Guard trigger: bypass quando extras_overridden = true
...
BEGIN
  ...
  -- Override manual permanente: fotógrafo assume controle
  IF COALESCE(NEW.extras_overridden, false) THEN
    RETURN NEW;
  END IF;
...
-- 2) RPC workflow_session_financials: respeita override + rateio proporcional sem galeria
...
BEGIN
  ...
  IF v_s.galeria_id IS NOT NULL AND NOT v_override THEN
    ...
    END IF;
  ELSE
    -- Sem galeria OU override manual ativo
    ...
    IF v_s.galeria_id IS NOT NULL AND v_override THEN
      -- Ainda captura v_gal_qtd para métricas comparativas
  ...
  IF v_pre_selecao THEN
    v_extras_pago := 0;
  ELSE
    -- (a) Componentes explícitos: cobranças com valor_extras_componente > 0
    SELECT COALESCE(SUM(
      CASE
    ...
    -- (b) Pagamentos genéricos (sem componente) — rateio proporcional
    -- só entra quando NÃO há galeria vinculada OU há override manual
