-- workflow_session_financials: modo explícito (gallery / override / manual / empty)
-- e fallback não-destrutivo da galeria por slug.
--
-- Motivação:
--   Sessões com extras manuais (sem galeria vinculada) estavam devolvendo
--   extras_pendente=0 quando existia uma galeria "órfã" com o mesmo slug em
--   status rascunho/enviado/selecao_iniciada — o RPC substituía v_s.galeria_id
--   pela galeria resolvida e caía no gate v_pre_selecao, zerando os extras
--   gravados na própria sessão.
--
-- Correção:
--   1) A galeria só é adotada quando NÃO há divergência com a sessão
--      (extras_overridden=false E qtd_fotos_extra=0 ou compatível com a
--      galeria).
--   2) Passa a existir um "modo de cálculo" explícito, evitando ambiguidade
--      futura entre "sem galeria" e "override".
--
-- Nenhuma mudança em outros triggers ou funções.
...
DECLARE
  ...
  v_extras_pago_manual numeric := 0;
...
BEGIN
  ...
  -- Fallback NÃO-DESTRUTIVO: só adota galeria por slug quando não há
  -- divergência com o que a sessão declara sobre extras.
  ...
  -- Modo de cálculo explícito.
  --   gallery  → galeria manda (comportamento tradicional).
  --   override → sessão manda mesmo com galeria vinculada.
  --   manual   → sem galeria, sessão tem extras próprios.
  --   empty    → sem galeria e sem extras.
  ...
  IF v_mode = 'gallery' THEN
    ...
    -- Gate pré-seleção: SÓ se aplica quando é o modo galeria puro.
  ...
  ELSIF v_mode = 'override' THEN
    ...
    IF v_s.galeria_id IS NOT NULL THEN
      SELECT COALESCE(g.total_fotos_extras_vendidas, 0) INTO v_gal_qtd
  ...
  ELSE
    ...
    SELECT COALESCE(SUM(
      ...
      INTO v_extras_pago_componentes
    ...
    -- (b) Pagamentos manuais rotulados via INTENT
    SELECT COALESCE(SUM(t.valor), 0)
      INTO v_extras_pago_manual
      FROM public.clientes_transacoes t
     WHERE t.user_id = v_s.user_id
       AND t.tipo = 'pagamento'
       ...
       AND (
         t.descricao ILIKE '%:fotos_extras]%'
         OR t.descricao ILIKE '%:sessao_e_extras]%'
       );

    -- (c) Rateio proporcional para pagamentos genéricos quando o modo é
    --     manual/override (sem cobrança explícita separando componentes).
    IF v_mode IN ('manual', 'override') AND v_total > 0 AND v_extras_liq > 0 THEN
      SELECT COALESCE(SUM(t.valor), 0)
       ...
       WHERE t.user_id = v_s.user_id
         ...
         AND (
           c.id IS NULL
           OR c.finalidade = 'sessao'
           OR (c.finalidade = 'sessao_e_extras'
               AND COALESCE(c.valor_sessao_componente, 0) + COALESCE(c.valor_extras_componente, 0) = 0)
         );

      v_extras_pago := v_extras_pago_componentes
                     + v_extras_pago_manual
                     + (v_pagamentos_genericos * (v_extras_liq / v_total));
    ELSE
      v_extras_pago := v_extras_pago_componentes + v_extras_pago_manual;
    END IF;

    v_extras_pago := LEAST(v_extras_liq, v_extras_pago);
  END IF;
