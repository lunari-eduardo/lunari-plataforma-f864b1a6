...
CREATE FUNCTION public.workflow_session_financials(p_session_id uuid)
...
DROP TRIGGER IF EXISTS trg_guard_qtd_fotos_extra_pre_selecao ON public.clientes_sessoes;
CREATE TRIGGER trg_guard_qtd_fotos_extra_pre_selecao
  BEFORE UPDATE OF qtd_fotos_extra, valor_total_foto_extra
  ON public.clientes_sessoes
  FOR EACH ROW
  EXECUTE FUNCTION public.guard_qtd_fotos_extra_pre_selecao();
...
UPDATE public.clientes_sessoes s
 ...
 WHERE s.galeria_id = g.id
   AND g.status IN ('rascunho', 'enviado', 'selecao_iniciada')
