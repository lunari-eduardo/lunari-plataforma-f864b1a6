-- CRITICAL SECURITY FIX: Recreate view with security_invoker=on
-- This ensures RLS policies on underlying tables are enforced per-user
...
FROM (((clientes_transacoes ct
    ...
    LEFT JOIN cobrancas cob ON (((cob.session_id = ct.session_id) AND (cob.user_id = ct.user_id) AND (cob.status = 'pago'::text))))
...
FROM (((clientes_transacoes ct
    ...
    LEFT JOIN clientes_sessoes cs ON (((ct.session_id = cs.session_id) AND (ct.user_id = cs.user_id))))
    LEFT JOIN cobrancas cob ON (((cob.session_id = ct.session_id) AND (cob.user_id = ct.user_id) AND (cob.status = 'pago'::text))))
WHERE ((ct.tipo = 'pagamento'::text) AND ((COALESCE(ct.taxa_gateway, (0)::numeric) + COALESCE(ct.taxa_antecipacao, (0)::numeric)) > (0)::numeric));