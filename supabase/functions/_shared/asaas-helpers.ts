...
export async function requestAsaasAnticipation(
  baseUrl: string,
  accessToken: string,
  paymentId?: string,
  ...
  try {
    const payload = installmentId ? { installment: installmentId } : { payment: paymentId };
    if (!installmentId && !paymentId) {
      return { ok: false, error: "Missing paymentId or installmentId" };
    }

    const res = await fetch(`${baseUrl}/v3/anticipations`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) {
      console.error("[Asaas] Failed to request anticipation:", data);
      return { ok: false, error: data };
    }

    console.log(`[Asaas] Anticipation requested for ${installmentId || paymentId}`);
    return { ok: true, data };
  } catch (err: any) {
    console.error("[Asaas] Exception in requestAsaasAnticipation:", err.message);
