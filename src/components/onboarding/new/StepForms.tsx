...
import { ClipboardList, Check, FileText, Info } from 'lucide-react';
...
interface StepFormsProps {
  ...
  onChange: (wantsForms: boolean, selectedSlugs: string[]) => void;
}

export function StepForms({ wantsForms, selectedSlugs, onChange }: StepFormsProps) {
  const handleDecision = (wants: boolean) => {
    if (wants) {
      const initial =
        selectedSlugs.length > 0 ? selectedSlugs : FORMULARIO_SEED_TEMPLATES.map((s) => s.slug);
    ...
    <div className="space-y-6 text-left animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="space-y-2">
        <h2 className="text-xl sm:text-2xl font-normal text-white tracking-tight">
          Você utiliza formulários?
        </h2>
        <p className="text-sm text-white/60 font-light leading-relaxed">
          Use formulários e briefings para coletar informações dos seus clientes antes ou durante o atendimento.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
        ...
        >
          <div className="w-10 h-10 rounded-xl bg-[#C6A36A]/15 border border-[#C6A36A]/30 flex items-center justify-center shrink-0">
            <ClipboardList className="w-5 h-5 text-[#C6A36A]" />
          </div>
          <div className="min-w-0">
            <p className="text-sm font-medium text-white">Sim, quero usar formulários</p>
            <p className="text-xs text-white/50 font-light">Começar com briefings prontos</p>
          </div>
        </button>
      ...
      {wantsForms === true && (
        <div className="space-y-4 pt-3 animate-in fade-in duration-200">
          ...
          <div className="space-y-2.5">
            {FORMULARIO_SEED_TEMPLATES.map((seed) => {
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-white">
                          {seed.nome.replace('Briefing — ', '')}
                        </p>
                        <span className="text-[10px] text-white/40 px-1.5 py-0.5 rounded bg-white/[0.04]">
                          {seed.campos.length} campos
                        </span>
                      </div>
