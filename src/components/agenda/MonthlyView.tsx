...
interface MonthlyViewProps {
  ...
  onCreateSlot: (slot: { date: Date }) => void;
...
export default function MonthlyView({
  ...
  return (
    <div className="w-full h-full overflow-auto">
      <div className="grid grid-cols-7 gap-px md:gap-1 rounded-lg p-px md:p-1 bg-border">
        {/* Weekday headers */}
        {weekDays.map((day) => (
          <div
            key={day}
            className="h-8 md:h-10 p-1 md:p-2 text-center text-xs font-medium text-muted-foreground rounded bg-muted"
          >
            {day}
          </div>
        ))}
        ...
        {emptyDaysBefore.map((_, index) => (
          <div
            key={`empty-${index}`}
            className="min-h-[80px] md:min-h-[120px] rounded bg-card"
          />
        ))}
        ...
        {daysInMonth.map((day) => {
          ...
          return (
            <div
              ...
              className="min-h-[80px] md:min-h-[120px] rounded p-1 md:p-2 cursor-pointer transition-colors bg-lunar-surface hover:bg-lunar-bg"
            >
