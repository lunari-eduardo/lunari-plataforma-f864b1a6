...
export default function PacoteForm({ 
  ...
  const adicionarProdutoIncluido = (produtoId: string) => {
    ...
    setFormData({
      ...formData,
      produtosIncluidos: [...formData.produtosIncluidos, { produtoId, quantidade: 1 }]
    });
  ...
  const atualizarQuantidadeProduto = (produtoId: string, quantidade: number) => {
    setFormData({
      ...formData,
      produtosIncluidos: formData.produtosIncluidos.map(p => 
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 bg-gray-50 rounded-lg">
        ...
        <div className="space-y-2">
          {formData.produtosIncluidos.map((produtoIncluido) => (
            ...
            <PopoverContent className="w-80 p-0">
              <Command>
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    {produtos
                      .filter(produto => !formData.produtosIncluidos.some(p => p.produtoId === produto.id))
                      .map((produto) => (
                      <CommandItem
                        key={produto.id}
