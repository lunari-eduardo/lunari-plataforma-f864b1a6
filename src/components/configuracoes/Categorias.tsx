...
interface CategoriasProps {
  ...
  pacotes: { categoria_id: string }[];
}

export default function Categorias({ categorias, setCategorias, pacotes }: CategoriasProps) {
    ...
    <div className="space-y-6 mt-4">
      ...
      <div>
        ...
        <div className="bg-white rounded-lg border border-gray-100 overflow-hidden shadow-sm">
          <div className="grid grid-cols-6 bg-gray-50/50 px-4 py-3 border-b border-gray-100">
          ...
          <div className="divide-y divide-gray-50">
            {categorias.map((categoria, index) => (
              <div 
                key={categoria.id} 
                className={`grid grid-cols-6 px-4 py-3 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25/30'} hover:bg-gray-50/70 transition-colors`}
              >
                    ...
                    <div className="flex justify-end col-span-3 gap-2">
                      <Button 
                        ...
                        onClick={() => salvarEdicaoCategoria(
                          categoria.id, 
                          categorias.find(c => c.id === categoria.id)?.nome || '', 
                          categorias.find(c => c.id === categoria.id)?.cor || ''
                        )}
                      >
                        Salvar
                      </Button>
