...
export default function LeadFormModal({ open, onOpenChange, mode, initial, onSubmit }: LeadFormModalProps) {
  ...
  const origens: OrigemCliente[] =
    contextOrigens.length > 0
      ? contextOrigens
  ...
  const handleAssociateClient = useCallback(
    (clienteId: string) => {
      if (!clienteId) {
                  ...
                  {origens.map((origem) => (
                    <SelectItem key={origem.id} value={origem.nome}>
                      <div className="flex items-center gap-2">
                        {origem.cor && <div className="w-3 h-3 rounded-full" style={{ backgroundColor: origem.cor }} />}
                        {origem.nome}
                      </div>
                    </SelectItem>
                  ))}
