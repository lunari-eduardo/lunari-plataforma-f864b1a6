...
export interface GalleryWelcomeModalProps {
  open: boolean;
  onClose: () => void;
  message: string;
  sessionName?: string;
...
export function GalleryWelcomeModal({
  open,
  onClose,
  message,
  sessionName,
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center transition-all ease-out ${
        stage === 'closing' ? 'duration-[2000ms] opacity-0 pointer-events-none' : 'duration-500 opacity-100'
      } ${overlayBg}`}
      style={{
