    ...
    <SectionShell tone="light">
      ...
      <Reveal delay={0.05}>
        ...
        >
          ...
          <span className="italic" style={{ color: "#C9A87C" }}>É o mesmo cérebro.</span>
        </h2>
      </Reveal>
          ...
          <div className="space-y-5" style={uiFont}>
            {points.map((p) => (
              <div key={p} className="flex items-start gap-3">
                <div className="mt-1 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-[#C9A87C]/12">
                  <Check className="h-3 w-3 text-[#C9A87C]" strokeWidth={2.5} />
                </div>
    ...
    >
      ...
      <div className="grid grid-cols-[1fr_180px]">
        <div className="grid grid-cols-3 gap-1.5 p-4">
          {Array.from({ length: 9 }).map((_, i) => {
            ...
            return (
              ...
              >
                {selected && (
                  <div className="absolute right-1.5 top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-[#C9A87C] text-white">
        ...
        <div className="border-l border-[#0A0A0A]/8 bg-[#FAFAF7]/30 p-4" style={uiFont}>
          ...
          <div className="mt-6 rounded-[8px] border border-[#C9A87C]/25 bg-[#C9A87C]/8 p-3">
            <div className="text-[10px] uppercase tracking-[0.14em] text-[#C9A87C]">
              +6 extras
            </div>
    ...
    <div className="flex items-center gap-2">
      <span
        className={`h-2 w-2 rounded-full ${
          done ? "bg-[#0A0A0A]" : active ? "bg-[#C9A87C]" : "bg-[#0A0A0A]/15"
        }`}
      />
