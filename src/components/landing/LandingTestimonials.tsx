  ...
  return <section className="py-20 bg-white/50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="md:text-4xl font-bold text-landing-text mb-4 text-2xl">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative bg-white rounded-2xl p-8 md:p-12 shadow-xl border border-landing-brand/10">
            <div className="text-center">
              ...
              }).map((_, i) => <Star key={i} className="w-6 h-6 fill-yellow-400 text-yellow-400" />)}
              </div>
            ...
            {/* Navigation */}
            <button onClick={prevTestimonial} className="absolute left-4 top-1/2 -translate-y-1/2 bg-landing-brand/10 hover:bg-landing-brand/20 rounded-full p-3 transition-all">
              <ChevronLeft className="w-5 h-5 text-landing-brand" />
            </button>
            <button onClick={nextTestimonial} className="absolute right-4 top-1/2 -translate-y-1/2 bg-landing-brand/10 hover:bg-landing-brand/20 rounded-full p-3 transition-all">
          ...
          <div className="flex justify-center mt-8 space-x-2">
            {testimonials.map((_, index) => <button key={index} onClick={() => setCurrentTestimonial(index)} className={`w-3 h-3 rounded-full transition-all ${currentTestimonial === index ? 'bg-landing-brand' : 'bg-landing-brand/30'}`} />)}
          </div>
        </div>
      </div>
    </section>;
