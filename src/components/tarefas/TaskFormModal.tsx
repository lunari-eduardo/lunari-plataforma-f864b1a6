...
export default function TaskFormModal({ open, onOpenChange, onSubmit, initial, mode = 'create' }: TaskFormModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleModalClose}>
        <DialogContent className="sm:max-w-xl bg-card dark:bg-foreground border-border/30 z-[200]">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="z-[10000] bg-card dark:bg-foreground border-border/30 w-[var(--radix-select-trigger-width,16rem)] min-w-[12rem]">
                {tagDefs.length ? (
                  tagDefs.map((tag) => (
