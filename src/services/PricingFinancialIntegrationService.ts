...
class PricingFinancialIntegrationService {
  ...
  // ============= INTEGRAÇÃO DE DEPRECIAÇÃO =============

  /**
   * Calcula a depreciação mensal total dos equipamentos
   */
  getEquipmentDepreciation(): {
    ...
    const equipamentosComDepreciacao = equipamentos.map(eq => ({
      id: eq.id,
      nome: eq.nome,
      valorPago: eq.valorPago,
      vidaUtil: eq.vidaUtil,
      depreciacaoMensal: eq.valorPago / (eq.vidaUtil * 12)
    }));

    const totalMensal = equipamentosComDepreciacao.reduce((total, eq) => total + eq.depreciacaoMensal, 0);

    return {
      totalMensal,
      equipamentos: equipamentosComDepreciacao
    };
  }

  /**
   * Sincroniza a depreciação mensal para o sistema financeiro
   */
  syncDepreciationToFinancial(): {
    success: boolean;
    itemFinanceiroId?: string;
    transacaoId?: string;
    ...
    try {
      const { totalMensal } = this.getEquipmentDepreciation();
      
      if (totalMensal <= 0) {
        return { 
          success: false, 
          valorDepreciacao: 0,
          error: 'Nenhuma depreciação para sincronizar' 
        };
      }

      // Buscar ou criar item "Depreciação de Equipamentos"
      const itensFinanceiros = storage.load(this.STORAGE_KEYS.FINANCIAL_ITEMS, []);
      let itemDepreciacao = itensFinanceiros.find((item: ItemFinanceiro) => 
        item.nome === 'Depreciação de Equipamentos' && 
        item.grupo_principal === 'Despesa Fixa'
      );
      ...
      if (!itemDepreciacao) {
        itemDepreciacao = {
          ...
          grupo_principal: 'Despesa Fixa' as GrupoPrincipal,
          userId: 'user1',
          ativo: true,
          criadoEm: getCurrentDateString()
        };

        itensFinanceiros.push(itemDepreciacao);
        storage.save(this.STORAGE_KEYS.FINANCIAL_ITEMS, itensFinanceiros);
      }

      // Criar transação mensal de depreciação
      const transacaoId = this.createDepreciationTransaction(itemDepreciacao.id, totalMensal);

      return {
        success: true,
        itemFinanceiroId: itemDepreciacao.id,
        transacaoId,
        valorDepreciacao: totalMensal
      };

    } catch (error) {
      console.error('Erro ao sincronizar depreciação:', error);
      return {
        success: false,
        valorDepreciacao: 0,
        error: error instanceof Error ? error.message : 'Erro desconhecido'
  ...
  /**
   * Cria transação mensal de depreciação
   */
  private createDepreciationTransaction(itemFinanceiroId: string, valor: number): string {
    const transacoes = RecurringBlueprintEngine.loadTransactions();
    const hoje = new Date();
    const mesAtual = hoje.getMonth() + 1;
    const anoAtual = hoje.getFullYear();

    // Verificar se já existe transação de depreciação para este mês
    const transacaoExistente = transacoes.find((t: any) => 
      t.itemId === itemFinanceiroId &&
      t.dataVencimento.includes(`${anoAtual}-${mesAtual.toString().padStart(2, '0')}`)
    );

    if (transacaoExistente) {
      // Atualizar valor da transação existente
      transacaoExistente.valor = valor;
      transacaoExistente.observacoes = `Depreciação automática - ${new Date().toISOString()}`;
      RecurringBlueprintEngine.saveTransaction(transacaoExistente);
    ...
    const novaTransacao = {
      id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
      itemId: itemFinanceiroId,
      valor: valor,
      dataVencimento: `${anoAtual}-${mesAtual.toString().padStart(2, '0')}-01`,
      status: 'Agendado' as const,
      observacoes: `Depreciação automática - ${new Date().toISOString()}`,
      criadoEm: getCurrentDateString(),
      userId: 'user1'
    };

    RecurringBlueprintEngine.saveTransaction(novaTransacao);
    
    return novaTransacao.id;
  }

  /**
   * Verifica se a depreciação está sincronizada
   */
  isDepreciationSynced(): {
    synced: boolean;
    valorPrecificacao: number;
    valorFinanceiro: number;
    diferenca: number;
    ...
    const itemDepreciacao = itensFinanceiros.find((item: ItemFinanceiro) => 
      item.nome === 'Depreciação de Equipamentos' && 
      item.grupo_principal === 'Despesa Fixa'
    );
    ...
    if (!itemDepreciacao) {
      return {
        synced: false,
        valorPrecificacao: totalMensal,
        valorFinanceiro: 0,
        diferenca: totalMensal
      };
    }

    // Verificar valor médio do item financeiro
    const valorFinanceiro = this.calculateAverageValueFromHistory(itemDepreciacao.id, 1) || 0;
    const diferenca = Math.abs(totalMensal - valorFinanceiro);
    const synced = diferenca < 0.01; // Tolerância de 1 centavo
