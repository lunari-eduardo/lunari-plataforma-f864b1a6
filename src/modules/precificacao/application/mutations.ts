...
import { calcularPrecoFinal, round2, validarFaixas, valorPorFoto } from "../domain/calculo";
...
const CustoInputSchema = z.object({
  descricao: z.string().optional(),
  valorUnitario: z.number(),
  quantidade: z.number().optional(),
});

function markupDaMargem(margemPercentual: number): number | null {
  const m = Number(margemPercentual) || 0;
  if (m <= 0 || m >= 100) return null;
  return round2(1 / (1 - m / 100));
}
const somaProdutos = (l?: z.infer<typeof ProdutoInputSchema>[]) =>
  (l ?? []).reduce((s, p) => s + (Number(p.custo) || 0) * (Number(p.quantidade) || 1), 0);
const somaCustos = (l?: z.infer<typeof CustoInputSchema>[]) =>
