...
| 4 — Actions + Queries | 🟢 quase completa | Capabilities **implementadas e registradas**: `workflow.advanceCard`, `workflow.updateFields`, `workflow.deleteSession`, `workflow.addPayment`, `workflow.refundPayment`, `workflow.getCardBySession`, `workflow.listMonth`, `workflow.statusOptions`, `workflow.search`, `workflow.metricsForMonth`, `workflow.pendingPayments`. Eventos `card_updated`/`card_deleted`/`payment_added`/`payment_refunded` no `LunariEvents`. **Migração de callsites**: `handleDeleteSession` → `workflow.deleteSession`; `handleAddPayment` → `ManualPaymentModal` + `workflow.addPayment`; **Onda 4a concluída** — `handleStatusChange` agora roteia por `workflow.advanceCard` e `updateSession` por `workflow.updateFields` para campos simples (desconto, valor_adicional, descrição, observações, categoria, status), atrás da flag `VITE_WORKFLOW_USE_CAPABILITY_UPDATE` (default on). Edições que exigem recongelamento (pacote, produtos, fotos extras) continuam no `useWorkflowRealtime` legado — orquestração pesada será portada em sub-onda futura. A capability `updateSessionFields` recalcula `valor_total` internamente usando `recalcFotosExtras`/`recalcSessionValorTotal` puras de `features/workflow/domain/pricing.ts` (mesmas funções já em uso pelo patch otimista da Page). **Pendente**: 4b — substituir canais legados `workflow-metrics-*` por listener do evento `workflow.card_updated`. **4c**: `createQuickSession`, `reconcileFotosExtras`, `syncFromAgenda`, `sessionTimeline`. **4d**: unificar `SessionPaymentsManager.addPayment` e `AppContext.addPayment` sobre `workflow.addPayment`. |
