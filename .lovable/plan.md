...
| 4 — Actions + Queries | 🟢 quase completa | Capabilities **implementadas e registradas**: `workflow.advanceCard`, `workflow.updateFields`, `workflow.deleteSession`, `workflow.addPayment`, `workflow.refundPayment`, `workflow.reconcileFotosExtras`, `workflow.createQuickSession`, `workflow.syncFromAgenda`, `workflow.getCardBySession`, `workflow.listMonth`, `workflow.statusOptions`, `workflow.search`, `workflow.metricsForMonth`, `workflow.pendingPayments`. **4a** concluída (status/updateFields via capability). **4b** concluída (métricas via eventBus, flag `VITE_WORKFLOW_METRICS_V2`). **4c** concluída — `reconcileFotosExtras` (wrapper RPC `reconcile_session_extras`), `createQuickSession` (insert direto em `clientes_sessoes`) e `syncFromAgenda` (espelha appointment confirmado, idempotente por `appointment_id`) disponíveis para IA/UI. **Pendente 4d**: unificar `SessionPaymentsManager.addPayment` e `AppContext.addPayment` sobre `workflow.addPayment`. |
