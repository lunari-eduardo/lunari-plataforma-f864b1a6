...
| 5 — Components + Hooks finos | 🟡 em andamento | **5a ✅** `Workflow.tsx` 1.318 → **251** linhas. Hooks finos em `features/workflow/hooks/`: `useWorkflowMonthSessions` (186), `useWorkflowFilters` (217), `useWorkflowColumns` (74), `useWorkflowSessionActions` (341 — alvo <250 na 5c). Componentes de apresentação: `WorkflowMetricsBar`, `WorkflowMonthSwitcher`, `WorkflowTasksDock`. Dead code removido (`recongelarTodasSessoes`, `prevMonthFinancials`, `renderPercentageChange`). Comportamento idêntico — typecheck limpo. **5b/5c/5d** pendentes (WorkflowTable, Cards, QuickSessionAdd). |
