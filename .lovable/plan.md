...
| 4 — Actions + Queries | 🟢 quase completa | Capabilities **implementadas e registradas**: `workflow.advanceCard`, `workflow.updateFields`, `workflow.deleteSession`, `workflow.addPayment`, `workflow.refundPayment`, `workflow.getCardBySession`, `workflow.listMonth`, `workflow.statusOptions`, `workflow.search`, `workflow.metricsForMonth`, `workflow.pendingPayments`. **Onda 4a concluída** — `handleStatusChange`/`updateSession` roteiam via capability atrás de `VITE_WORKFLOW_USE_CAPABILITY_UPDATE`. **Onda 4b concluída** — hooks `useWorkflowMetricsRealtime` e `useWorkflowMetricsByYear` reagem ao `eventBus` (`workflow.card_*`/`workflow.payment_*`) + `CustomEvent('workflow-session-updated'|'workflow-session-deleted')` em vez de canais Supabase próprios. Flag `VITE_WORKFLOW_METRICS_V2` (default on) para rollback — quando `false` reativa os canais legados. **Pendente**: 4c — capabilities `createQuickSession`, `reconcileFotosExtras`, `syncFromAgenda`. **4d**: unificar `SessionPaymentsManager.addPayment` e `AppContext.addPayment` sobre `workflow.addPayment`. |
