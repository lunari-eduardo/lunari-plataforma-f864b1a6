...
| 4 — Actions + Queries | 🟢 quase completa | Capabilities **implementadas e registradas**: `workflow.advanceCard`, `workflow.updateFields`, `workflow.deleteSession`, `workflow.addPayment`, `workflow.refundPayment`, `workflow.getCardBySession`, `workflow.listMonth`, `workflow.statusOptions`, `workflow.search`, `workflow.metricsForMonth`, `workflow.pendingPayments`. Eventos `card_updated`/`card_deleted`/`payment_added`/`payment_refunded` no `LunariEvents`. **Migração de callsites (parcial — sem mudança de UX)**: `handleDeleteSession` consome `workflow.deleteSession`; `handleAddPayment` agora abre `ManualPaymentModal` (`src/components/workflow/ManualPaymentModal.tsx`) que escreve via `workflow.addPayment` (idempotente, auditável, evento `workflow.payment_added`). O `SessionPaymentsManager` (cards expansíveis) segue como antes — sub-onda futura unifica. **Pendente**: `updateSession`/`handleStatusChange` ainda usam `useWorkflowRealtime` por dependerem do recálculo otimista (`recalcFotosExtras`/`recalcSessionValorTotal`) e refreezing — migrar p/ `workflow.updateFields` exige mover essa lógica para dentro da capability sem regredir UX. **Sub-onda 4c (novas capabilities)**: `createQuickSession`, `reconcileFotosExtras`, `syncFromAgenda`, `sessionTimeline`. **Sub-onda 4d**: unificar `SessionPaymentsManager.addPayment` e `AppContext.addPayment` (input rápido) sobre `workflow.addPayment` (checklist: parcelas, agendado, estorno, origem). |
