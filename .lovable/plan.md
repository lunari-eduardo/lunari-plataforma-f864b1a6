...
| 5 — Components + Hooks finos | 🟡 em andamento | **5a ✅** `Workflow.tsx` 1.318 → **251** linhas. **5b ✅** `WorkflowTable` 825 → **72** linhas (shim sobre `WorkflowCardList`). Auditoria revelou que toda a tabela HTML interna era dead code: o `return` sempre montou apenas `<WorkflowCardList>` desde a migração para layout de cards — ~750 linhas de inputs editáveis, redimensionamento de colunas, scroll horizontal contínuo, `ExtraPhotoQtyInput` duplicado e cálculo client-side foram removidas. Interface pública preservada (props legadas como `visibleColumns`/`columnWidths`/`onSort` aceitas e ignoradas) para não quebrar `Workflow.tsx`. Typecheck limpo. **5c/5d** pendentes (Cards 594+632, QuickSessionAdd 776 + GerenciarProdutosModal 479). |
