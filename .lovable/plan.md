...
| 4 — Actions + Queries | 🟡 parcial | Capabilities **implementadas e registradas**: `workflow.advanceCard`, `workflow.updateFields`, `workflow.deleteSession`, `workflow.addPayment`, `workflow.getCardBySession`, `workflow.listMonth`, `workflow.statusOptions`. Eventos `card_updated`/`card_deleted`/`payment_added` adicionados ao `LunariEvents`. **Pendente**: `refundPayment`, `createQuickSession`, `reconcileFotosExtras`, `syncFromAgenda`, `search`, `metricsForMonth`, `pendingPayments`, `sessionTimeline` + migração dos callsites em `Workflow.tsx`. |
