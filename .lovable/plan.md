...
| 4 — Actions + Queries | 🟢 quase completa | Capabilities **implementadas e registradas**: `workflow.advanceCard`, `workflow.updateFields`, `workflow.deleteSession`, `workflow.addPayment`, `workflow.refundPayment`, `workflow.getCardBySession`, `workflow.listMonth`, `workflow.statusOptions`, `workflow.search`, `workflow.metricsForMonth`, `workflow.pendingPayments`. Eventos `card_updated`/`card_deleted`/`payment_added`/`payment_refunded` no `LunariEvents`. **Migração de callsites (parcial — sem mudança de UX)**: `handleDeleteSession` em `Workflow.tsx` agora consome `workflow.deleteSession` (toasts e fluxo idênticos). **Pendente — exige UI nova (deixar para etapa posterior)**: (a) `handleAddPayment` é `console.log` → precisa modal de pagamento manual antes de plugar `workflow.addPayment`; (b) `updateSession`/`handleStatusChange` ainda usam `useWorkflowRealtime` por dependerem do recálculo otimista (`recalcFotosExtras`/`recalcSessionValorTotal`) e refreezing — migrar p/ `workflow.updateFields` exige mover essa lógica para dentro da capability (ou hook fino) sem regredir a UX. **Pendente — sub-onda 4c (novas capabilities)**: `createQuickSession`, `reconcileFotosExtras`, `syncFromAgenda`, `sessionTimeline`. |
